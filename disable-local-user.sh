...
# This script disables, delets and/or archives users on the local system.
...
usage() {
  ...
  echo '  -d Deletes accounts instrad of disabling them.' >&2
...
do 
  ...
  then
    ...
    echo "${HOME_DIR} does not exist or if not a directory." >&2
    exit 1
  fi
fi
